...
public class LevelGridSystemVisualSingle : MonoBehaviour //Сеточная система визуализации еденицы сетки (ячейки) // Лежит на самом префабе (белая рамка)
{
    ...
    public void ShowСircleGrenade(float radius) // Показать Круг нужного диаметра
    {
        _meshRendererСircleGrenade.enabled = true;
        _meshRendererСircleGrenade.transform.localScale = Vector3.one* 2*radius;
    }

    public void HideСircleGrenade() // Скрыть
    {
        _meshRendererСircleGrenade.enabled = false;
    }

    public void ShowQuadGrenade(Material material, float radius = 1) // Показать Квадрат нужного диаметра
    {
    ...
    public void HideQuadGrenade() // Скрыть
    {
