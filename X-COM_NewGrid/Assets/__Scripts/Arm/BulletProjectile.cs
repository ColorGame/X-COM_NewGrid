...
public class BulletProjectile : MonoBehaviour // —нар€д пули
{
    ...
    private bool _hit; // ѕопал или промазал

    public void Setup(Vector3 targetPosition, bool hit) // Ќастроика пули
    {
    ...
    private void Update()
    {
        ...
        //float distanceBeforeMoving = Vector3.Distance(transform.position, _targetRotation); //(рассто€ние до движени€) Ќайдем и сохраним расто€ние до целевого юнита, прежде чем начнем двигатьс€
        //float moveSpead = 200f;
        //transform.position += moveDirection * moveSpead * Time.deltaTime; // переместим пулю
        //float distanceAfterMoving = Vector3.Distance(transform.position, _targetRotation); //(рассто€ние после движени€) Ќайдем и сохраним расто€ние до целевого юнита, после начала движени€
        ...
        if (sqrMagnitudeBeforeMoving < sqrMagnitudeAfterMoving) // if сработает, когда мы перелетим цель, и отдалимс€ от нее, на рассто€ние больше, чем когда мы были до цели. Ѕлиже к цели эти рассто€ни€ становитьс€ мальнекими. ≈сли ввести stoppingDistance как в MoveAction из за большой скорости пул€ будет долго колебатьс€ возле цели пролета€ дистанцию остановки.
        {
            ...
            if (_hit) //≈сли попали то создадим частички
            {
                Instantiate(GameAssets.Instance.bulletHitFXPrefab, _targetPosition, Quaternion.identity); // —оздадим префаб частиц (Ќе забудь в инспекторе включить у частиц Stop Action - Destroy)
            }
        }
    }
}
