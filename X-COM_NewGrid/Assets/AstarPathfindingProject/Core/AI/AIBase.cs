...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		void Init () {
			if (startHasRun) {
                // ѕрив€зать агента к navmesh (что, по сути, и будет делать вызов телепорта. ’от€ только некоторые сценарии перемещени€ требуют этого, например RichAI).
                // ¬ызов телепорта также обеспечит правильную инициализацию некоторых переменных(like #prevPosition1 and #prevPosition2)
		...
		protected virtual void Update () {
			if (shouldRecalculatePath) SearchPath(); //≈сли следует пересчитать путь

            // ≈сли используетс€ гравитаци€, это зависит от многих вещей.
            // Ќапример, когда используетс€ некинематическое жесткое тело, то жесткое тело само будет примен€ть силу т€жести
            // ќбратите внимание, что гравитаци€ может содержать NaN, вот почему при сравнении используетс€ !(a==b) вместо просто a!=бит.
