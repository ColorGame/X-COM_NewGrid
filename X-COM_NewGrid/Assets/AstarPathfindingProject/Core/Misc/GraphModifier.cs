...
namespace Pathfinding {
	...
	public abstract class GraphModifier : VersionedMonoBehaviour {
        ...
        /// <summary>
        /// Вызывается сразу после того, как все графики были отсканированы.
        /// Заливка флудом и другая последующая обработка не производились.
        ///
        /// Предупреждение: Поскольку OnEnable и Awake вызываются примерно в одно и то же время, единственный способ
        /// чтобы гарантировать, что эти скрипты получат этот вызов при сканировании в режиме бодрствования, необходимо
        /// установить порядок выполнения скрипта для AstarPath на некоторое время позже времени по умолчанию
        /// (смотрите раздел Правка -> Настройки проекта -> Порядок выполнения скрипта).
        /// ЧТО ДЕЛАТЬ: это все еще актуально? Вызов для поиска всех модификаторов должен быть выполнен до вызова этого метода
        /// таким образом, приведенное выше предупреждение, вероятно, больше не актуально.
        ///
        /// See: OnLatePostScan
        /// </summary>
