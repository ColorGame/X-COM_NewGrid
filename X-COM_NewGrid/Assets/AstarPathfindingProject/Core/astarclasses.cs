...
namespace Pathfinding {
    ...
    /// <summary>Ограничение ближайшего узла. Ограничения, какие узлы будут возвращены с помощью<see cref="AstarPath.GetNearest"/> function</summary>
    public class NNConstraint {
        ...
        /// <summary>Возвращает ограничение, которое не фильтрует результаты</summary>
    ...
    /// <summary>
    /// Внутренний результат запроса к ближайшему узлу.
    /// See: NNInfo
    /// </summary>
    ...
    /// <summary>Результат запроса к ближайшему узлу</summary>
    public struct NNInfo {
        /// <summary>Ближайший узел</summary>
        public readonly GraphNode node;

        /// <summary>
        /// Ближайшая точка на навигационной карте.
        /// Это позиция запроса, привязанная к ближайшей точке на <see cref="node"/>.
        /// </summary>
        public readonly Vector3 position;

        /// <summary>
        /// Ближайшая точка на навигационной карте.
        /// Устарело: это поле было переименовано в <see cref="position"/>
        /// </summary>
	...
	public class GraphUpdateObject {
        /// <summary>
        /// Границы для обновления узлов внутри.
        /// Определен в мировом пространстве.
        /// </summary>
        public Bounds bounds;

        /// <summary>
        /// Контролирует, будет ли произведена заливка после нанесения этого состава.
        /// Отключение этого параметра можно использовать для повышения производительности, но используйте его с осторожностью.
        /// Если вы уверены, что СТАТУС-КВО не изменит проходимость или соединения. Вы можете установить для этого значение false.
        /// Например, при обновлении только значений штрафных санкций можно сэкономить вычислительную мощность, установив для этого значение false. Особенно на больших графиках.
        /// Примечание: Если вы установите для этого значение false, даже если это изменит, например, проходимость, это может привести к возвращению путей, которые не прошли, даже если путь есть,
        /// или они пытаются найти путь по всему графику, даже если его нет, и в процессе будут использоваться огромные вычислительные мощности.
        ///
        ///При использовании базового GraphUpdateObject (не является производным классом), быстрый способ проверить, потребуется ли заливка, - это проверить, требуется ли <see cref="modifyWalkability"/> is true or <see cref="updatePhysics"/> is true.
        ///
        /// Устарело: больше не требуется
        /// </summary>
    ...
    /// Использует включающий диапазон координат.
    ///
    /// Работает почти как UnityEngine.Rect, но с целочисленными координатами
    /// </summary>
    [System.Serializable]
    ...
    /// <summary>
    /// Делегат с объектом on Path в качестве параметра.
    /// Это используется для обратных вызовов, когда вычисление пути завершено.
    /// Пример функции:
    /// <code>
    /// public void Start () {
    ...
    ///
    ///     // seeker.pathCallback является OnPathDelegate, мы добавляем к нему функцию OnPathComplete, чтобы она вызывалась всякий раз, когда завершается вычисление пути для этого искателя
    ///     seeker.pathCallback += OnPathComplete;
    /// }
    ///
    /// public void OnPathComplete (Path p) {
    ///     Debug.Log("Это вызывается, когда путь завершен для искателя, прикрепленного к этому GameObject");
    /// }
    /// </code>
    /// </summary>
    ...
    /// <summary>Внутреннее состояние пути в трубопроводе</summary>
