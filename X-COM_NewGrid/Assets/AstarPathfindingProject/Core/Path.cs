...
namespace Pathfinding {
    /// <summary>
    /// Предоставляет дополнительную информацию об обходе для запроса пути.
    /// Смотрите: пошагово (рабочие ссылки смотрите в онлайн-документации)
    /// </summary>
    ...
    /// <summary>Удобный класс для доступа к реализации по умолчанию ITraversalProvider</summary>
    ...
    /// <summary>Базовый класс для всех типов путей</summary>
    public abstract class Path : IPathInternals {
        ...
        /// <summary>Данные для потока, вычисляющего этот путь</summary>
        protected PathHandler pathHandler;

        /// <summary>
        /// Обратный вызов для вызова, когда путь будет завершен.
        /// Обычно это отправляется компоненту Seeker, который post обрабатывает путь, а затем вызывает обратный вызов скрипту, который запросил путь
        /// </summary>
        ...
        /// <summary>Возвращает состояние пути в конвейере поиска пути</summary>
        ...
        /// Ошибка: В настоящее время это может быть настроено на завершение до того, как путь будет фактически полностью вычислен. В частности, векторный путь и списки путей, возможно, были созданы не полностью.
        /// Это может привести к возникновению условий гонки при использовании многопоточности. Старайтесь не использовать этот метод для проверки того, вычислен ли путь прямо сейчас, использовать <see cref="IsDone"/> instead.
        /// </summary>
        ...
        /// <summary>
        /// Содержит путь в виде массива узлов. Все узлы, через которые проходит путь.
        /// Это могут быть не те же узлы, которые пересекает постобработанный путь.
        /// </summary>
        public List<GraphNode> path;

        /// <summary>Содержит (возможно, после обработки) путь в виде списка Vector3</summary>
        public List<Vector3> vectorPath;

        /// <summary>Обрабатываемый в данный момент узел</summary>
        protected PathNode currentR;

		/// <summary>How long it took to calculate this path in milliseconds</summary>
		public float duration;

        /// <summary>Количество узлов, в которых был выполнен поиск по этому пути</summary>
        public int searchedNodes { get; protected set; }

        /// <summary>
        /// Истинно, если путь в данный момент объединен.
        /// Не устанавливайте это значение. Только читать. Он используется внутри компании.
        ///
        /// /// Смотрите: Patchpool
        /// Версия: Была названа "recycled-переработанный" в версии 3.7.5 и более ранних версиях.
        /// </summary>
        bool IPathInternals.Pooled { get; set; }

        /// <summary>
        /// Истинно, если путь в данный момент повторно используется (т.е. в пуле путей).
        /// Не устанавливайте это значение. Только читать. Он используется внутри компании.
        ///
        /// Устаревший: был переименован в "pooled-объединенный", чтобы использовать более широко понятную терминологию
        /// </summary>
        ...
        /// <summary>
        /// Истинно, если была вызвана функция сброса.
        /// Используется для оповещения пользователей, когда они делают что-то не так.
        /// </summary>
        ...
        /// <summary>
        /// Реализация внутреннего связанного списка.
        /// Предупреждение: Это используется системой внутри системы. Вы никогда не должны этого менять.
        /// </summary>
        internal Path next;
        ...
        /// <summary>
        /// Какие теги графа доступны для просмотра.
        /// Это битовая маска, поэтому -1 = все биты установлены = все теги доступны для просмотра.
        /// Например, чтобы установить бит 5 в значение true, вы должны выполнить
        /// <код> myPath.enabled Tags |= 1 << 5; </код>
        /// Чтобы установить для него значение false, вы должны выполнить
        /// <code> myPath.enabled теги &= ~(1 << 5); </ код>
        ///
        /// У (искателя) есть всплывающее поле, в котором вы можете указать, какие теги использовать.
        /// Примечание: Если вы используете a (искатель). Искатель установит это значение равным тому, которое задано в поле inspector на (StartPath).
        /// Итак, вам нужно изменить значение искателя с помощью скрипта, а не устанавливать это значение, если вы хотите изменить его с помощью скрипта.
        ///
        /// See: CanTraverse
        /// </summary>
        ...
        /// <summary>
        /// Фактически применяемые налоговые санкции.
        /// Если значение ручных налоговых штрафов равно нулю, это будет ZeroTagPenalties
        /// See: tagPenalties
        /// </summary>
        protected int[] internalTagPenalties;

        /// <summary>
        ///Штрафы за теги, установленные другими скриптами
        /// See: tagPenalties
        /// </summary>
        ...
        /// Тег 0, который является тегом по умолчанию, добавит штраф в виде налоговых санкций[0].
        /// Это должны быть только положительные значения, поскольку алгоритм A* не может обрабатывать отрицательные штрафные санкции.
        ///
        /// При присвоении массива этому свойству его длина должна быть равна 32.
        ///
        /// Примечание: Установка значения null или попытка присвоить массив, длина которого не равна 32, приведет к тому, что все налоговые штрафы будут обрабатываться так, как если бы они были равны нулю.
        ///
        /// Примечание: Если вы используете поисковик. Искатель установит это значение равным тому, которое задано в поле инспектор при вызове искателя.Начальный путь.
        /// Итак, вам нужно изменить значение искателя с помощью скрипта, а не устанавливать это значение.
        ///
        ...
        /// <summary>
        /// Верно для путей, которые хотят выполнять поиск по всем узлам, а не перепрыгивать через узлы в качестве оптимизации.
        /// Это отключает поиск точки перехода, когда он включен, чтобы предотвратить, например, постоянный путь и путь затопления
        ...
        /// <summary>
        /// Ожидает, пока этот путь не будет вычислен и возвращен.
        /// Позволяет создавать очень простые сценарии.
        /// <code>
        /// IEnumerator Start () {
        ...
        /// </code>
        ///
        /// Примечание: Не путайте это с AstarPath.WaitForPath. Этот параметр будет ждать, используя yield, до тех пор, пока он не будет рассчитан
        /// пока AstarPath.WaitForPath остановит все операции до тех пор, пока путь не будет вычислен.
        ///
        /// Бросает: System.InvalidOperationException если путь не запущен. Пошли путь Seeker.StartPath или AstarPath.StartPath перед вызовом этой функции.
        ///
        /// See: <see cref="BlockUntilCalculated"/>
        /// See: https://docs.unity3d.com/Manual/Coroutines.html
        /// </summary>
        public IEnumerator WaitForPath () {
			if (PipelineState == PathState.Created) throw new System.InvalidOperationException("Этот путь еще не начат");

			while (PipelineState != PathState.Returned) yield return null;
		}

        /// <summary>
        /// Блокирует до тех пор, пока этот путь не будет вычислен и возвращен.
        ...
        /// Это сглаживает частоту кадров и гарантирует, что запрос большого
        /// количество путей одновременно не приводит к задержке.
        ///
        ...
        /// </code>
        ///
        /// See: Это эквивалентно вызову AstarPath.BlockUntilCalculated(Path)
        /// See: WaitForPath
        /// </summary>
        ...
        /// <summary>
        /// Возвращает, если узел может быть пройден.
        /// Это значение по умолчанию равно, если узел является walkabble и если тег nodes включен в<see cref="enabledTags"/>
        /// </summary>
        public bool CanTraverse (GraphNode node) {
            // Используйте поставщик обхода, если он установлен, в противном случае вернитесь к поведению по умолчанию
            // Этот метод популярен, но эта ветвь чрезвычайно хорошо предсказуема, так что он
            // не сильно влияет на производительность (профилирование указывает, что она чуть выше
            // уровень шума, где-то около 0%-0,3%)
            if (traversalProvider != null)
				return traversalProvider.CanTraverse(this, node);

            // Вставленный вручную код из DefaultITraversalProvider
        ...
        /// <summary>
        /// Истинно, если вычисление этого пути завершено.
        ///
        /// Примечание: Обратный вызов для пути, возможно, еще не был вызван.
        ///
        /// Since: Added in 3.0.8
        ///
        /// See: <see cref="Seeker.IsDone"/> который также учитывает, был ли вызван обратный вызов path и применены ли модификаторы.
        /// </summary>
        public bool IsDone () {
