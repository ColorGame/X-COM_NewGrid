...
namespace Pathfinding
{
        ...
        , IRaycastableGraph
    {
        ...
        /// <summary>
        /// Количество слоев на графике.
        /// Для сеточных графиков это значение всегда равно 1, для многоуровневых сеточных графиков оно может быть выше.
        /// Массив узлов имеет размер ширина*глубина*количество слоев.
        /// </summary>
        ...
        /// <summary>Ширина сетки в узлах. See: SetDimensions</summary>
        public int width;

        /// <summary>Глубина (высота) сетки в узлах. See: SetDimensions</summary>
        public int depth;

        /// <summary>
        /// Масштабирование графика по оси X.
        /// Это следует использовать, если вам нужны разные масштабы по осям X и Y сетки
        /// </summary>
        [JsonMember]
        ...
        /// <summary>
        /// Если значение равно true, то для всех граничных затрат будет установлено одинаковое значение.
        /// Если значение false, диагонали будут стоить дороже.
        /// Это полезно для шестиугольного графика, где ромбы на самом деле имеют ту же длину,нормальные ребра (поскольку график был искажен)
        /// </summary>
        [JsonMember]
        ...
        /// <summary>
        ///Размер одного узла в мировых единицах измерения.
        ...
        /// <summary>
        /// Смещение индекса для получения соседних узлов. Добавляется к индексу узла, чтобы получить индекс соседнего узла.
        ///
        /// <code>
        ///         Z
        ///         |
        ...
        /// <summary>
        /// Все узлы на этом графике.
        /// Узлы располагаются ряд за рядом.
        ///
        /// Первый узел имеет координаты сетки X=0, Z=0, второй узел X=1, Z=0
        /// последний имеет координаты сетки X=width(ширина)-1, Z=depth(глубина)-1.
        ///
        /// <code>
        ...
        /// <summary>
        /// Преобразуйте точку в пространстве графа в мировое пространство.
        /// Это даст вам положение узла для узла в заданных координатах x и z
        /// если он находится на указанной высоте над основанием графика.
        /// </summary>
        ...
        public virtual void SetUpOffsetsAndCosts()
        {
            ...
            // Диагонали обычно стоят в sqrt(2) (приблизительно в 1,41) раза дороже
           // uint diagonalCost = uniformEdgeCosts ? straightCost : (uint)Mathf.RoundToInt(nodeSize * Mathf.Sqrt(2F) * Int3.Precision);
            uint diagonalCost = uniformEdgeCosts ? straightCost : (uint)Mathf.RoundToInt(nodeSize * 2f * Int3.Precision); // Ромб для хотьбы // Изменил
        ...
        /// <summary>
        /// Истинно, если у узла есть какие-либо заблокированные соединения.
        /// Для 4-х и 8-ми соседних элементов будут проверены соединения, выровненные по 4 осям.
        /// Для 6 соседей будут проверены все 6 соседей.
        ///
        ...
        /// <summary>
        /// Вычисляет сетевые соединения для ячейки, а также для ее соседей.
        /// Это полезная служебная функция, если вы хотите изменить проходимость одного узла на графике.
        ///
        /// <code>
        ...
        public void CalculateConnectionsForCellAndNeighbours(int x, int z)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                // Проверьте, находится ли новая позиция внутри сетки
                // Побитовое И (&) значительно быстрее, чем &&
                // (ненамного, но этот код горячий)
                if (nx >= 0 & nz >= 0 & nx < width & nz < depth)
                {
