...
namespace Pathfinding {
    /// <summary>
    /// Путь, который ведет поиск от одной точки к нескольким различным целям за один поиск или от нескольких разных начальных точек к одной цели.
    ///
    /// Это быстрее, чем поиск с помощью ABPath для каждой цели, если значение pathsForAll равно true.
    /// Этот тип пути можно использовать, например, когда вы хотите, чтобы агент нашел ближайшую цель из нескольких различных вариантов.
    ///
    /// Когда pathsForAll имеет значение true, он вычислит путь к каждой целевой точке, но он может совместно использовать множество вычислений для разных путей, поэтому
    /// это быстрее, чем запрашивать их по отдельности.
    ///
    /// Когда значение pathsForAll равно false, он выполнит поиск с использованием эвристики, имеющей значение None, и остановится, как только найдет первую цель.
    /// Это может быть быстрее или медленнее, чем запрос каждого пути по отдельности.
    /// Он выполнит поиск Дейкстры, где он выполняет поиск по всем узлам вокруг начальной точки, пока не будет найдена ближайшая цель.
    /// Обратите внимание, что обычно это происходит быстрее, если некоторые целевые точки находятся очень близко к начальной точке, а некоторые - очень далеко, но
    /// это может быть медленнее, если все целевые точки находятся относительно далеко, потому что тогда ему придется искать гораздо больший
    /// region, поскольку он не будет использовать никаких эвристик.
    ///
