...
namespace Pathfinding {
    /// <summary>
    /// Базовый путь, находит кратчайший путь от A до B.
    ///
    /// Это самый простой объект path, он попытается найти кратчайший путь между двумя точками.
    /// Многие другие типы путей наследуются от этого типа.
    ...
    public class ABPath : Path {
        ...
        /// <summary>
        /// Постройте путь с начальной и конечной точками.
        /// Делегат будет вызван, когда путь будет вычислен.
        /// Не путайте это с обратным вызовом Seeker, поскольку они отправляются в разное время.
        /// Если вы используете искатель для начала пути, вы можете установить обратный вызов равным null.
        ///
        /// Возвращает: Построенный объект path
        /// </summary>
        ...
        /// <summary>Вспомогательный метод для установки Path Node.flag 1 в определенное значение для всех узлов, смежных с узлом сетки</summary>
        void SetFlagOnSurroundingGridNodes (GridNode gridNode, int flag, bool flagState) {
            // Цикл по всем соседним узлам сетки
            var gridGraph = GridNode.GetGridGraph(gridNode.GraphIndex);

            // Количество соседей в виде int
			...
			for (int i = 0; i < mxnum; i++) {
                ...
                // Проверьте, находится ли позиция по-прежнему внутри сетки
